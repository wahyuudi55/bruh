...
const os = require('os');
const fs = require('fs');
const whyx = require('./function/index') 
...
app.get('/api/ragbot', async (req, res) => {
  try {
    ...
    const response = await whyx.ragBot(message);
    res.status(200).json({
      status: 200,
      creator: "siputzx",
      data: { response }
...
app.get('/api/degreeguru', async (req, res) => {
  try {
    ...
    const response = await whyx.degreeGuru(message);
    res.status(200).json({
      status: 200,
      creator: "whyuxD",
      data: { response }
...
app.get('/api/tiktok', async (req, res) => {
  try {
    ...
    const response = await whyx.tiktok(link);
    res.status(200).json({
      status: 200,
      creator: "whyuxD",
      data: { response }
...
app.get('/api/smartcontract', async (req, res) => {
  try {
    ...
    const response = await whyx.smartContract(message);
    res.status(200).json({
      status: 200,
      creator: "whyuxD",
      data: { response }
...
app.get('/api/blackboxAIChat', async (req, res) => {
  try {
    ...
    const response = await whyx.blackboxAIChat(message);
    res.status(200).json({
      status: 200,
      creator: "whyuxD",
      data: { response }
